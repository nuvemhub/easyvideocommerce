...
var easyDataLayer = {
  ...
  sendAnalyticsEvent: function (eventType, data) {
    easyDataLayer.utils.executeWithLogging(() => {
      ...
      const pushData = {
        ...
        'easyvc_campaignid': easyDataLayer.analytics.campaignId,
        'easyvc_etype': eventType,
        ...(easyDataLayer.analytics.testId && { easyvc_testid: easyDataLayer.analytics.testId }),
  ...
  handleWppContact: function () {
    easyDataLayer.utils.executeWithLogging(() => {
      ...
      window.open(
        `https://wa.me/${easyDataLayer.config.wppNumber}?text=${encodeURIComponent(helloWppText)}`,
        '_blank'
      );
    }, 'handleWppContact');
  },
  ...
  getEasyCampaigns: function (isHome, categorieId, productId, productHandle, paths) {
    return new Promise((resolve) => {
      const params = [
        isHome ? `isHome=${isHome}` : null,
        categorieId ? `categoryId=${categorieId}` : null,
        productId ? `productId=${productId}` : null,
        productHandle ? `productHandle=${productHandle}` : null,
        paths ? `paths=${encodeURIComponent(JSON.stringify(paths))}` : null,
      ].filter(Boolean).join('&');
  ...
  main: async function () {
    easyDataLayer.utils.executeWithLogging(async () => {
      ...
      // NuvemShop
      if (window?.LS) {
        if (window?.LS?.category?.id) {
          categorieId = window.LS.category.id;
        } else if (window?.LS?.product?.id) {
          productId = window.LS.product.id;
        } else if (window.location.pathname?.match('/')) {
          const splitPaths = window.location.pathname.split('/');
          const filter = splitPaths.filter(path => path !== "");
          if (filter.length > 0) {
            paths = filter;
          }
        }
      ...
      if (window?.Shopify) {
        if (window.location.pathname?.match('/collections')) {
          const splitPaths = window.location.pathname.split('/');
          const filter = splitPaths.filter(path => path !== "" && path !== "collections");
          if (filter.length > 0) {
            paths = filter[0];
          }
        } else if (window.location.pathname?.match('/products')) {
          const splitPaths = window.location.pathname.split('/');
          const filter = splitPaths.filter(path => path !== "" && path !== "products");
          if (filter.length > 0) {
            productHandle = filter[0];
          }
        } else if (window.location.pathname?.match('/')) {
          const splitPaths = window.location.pathname.split('/');
          const filter = splitPaths.filter(path => path !== "");
      ...
      if (productId) easyDataLayer.config.productId = productId;

      // invalid body
      if (!isHome && !categorieId && !productId && !productHandle && !paths) return;

      const result = await easyDataLayer.getEasyCampaigns(isHome, categorieId, productId, productHandle, paths);
      ...
      if (result?.store?.app) {
        easyDataLayer.store = result.store;
        easyDataLayer.store.app = result.store.app;
      }
      ...
      if (result?.store?.app?.personalization?.color) {
        easyDataLayer.setCustomColor(result.store.app.personalization.color);
      }
